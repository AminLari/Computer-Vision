...
Hdilated = imdilate(H, ones(20,20,4));

% We want those places where H = Hdialated and H > THRESH
THRESH = 0.95 * max(H(:));
Hpeaks = (H == Hdilated) & (H > THRESH);

% Find indices of the peaks (the nonzero points). 
[iRow,iCol,iDepth] = ind2sub(size(Hpeaks), find(Hpeaks));

% draw parabolas
figure, imshow(I), title('Parabola detected using Hough Transform');;
for i=1:length(iRow)
    x0 = iCol(i);
    y0 = iRow(i);
    r0 = C(iDepth(i));
